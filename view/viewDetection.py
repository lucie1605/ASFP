...
class ViewDetection:
    ...
    def __init__(self, root, model):
        ...
        self.label_image_loaded = tk.StringVar()
        ...
        tk.Label(self.root, textvariable=self.label_image_loaded).grid(row=0, column=2, padx=2, pady=2)
    ...
    def load_img(self):
        ...
        self.label_image_loaded.set(f"1 image loaded : {self.picture}")

    def load_directory(self):
        self.directory_path = filedialog.askdirectory()
        self.label_image_loaded.set(f"1 folder loaded : {self.directory_path}")

    def show_image_after_detection(self, image, row=1, column=0):
        blue, green, red = cv2.split(image)
        img = cv2.merge((red, green, blue))
        img = Image.fromarray(img)
        img.thumbnail(ViewDetection.FORMAT_IMAGE_OUTPUT, Image.ANTIALIAS)
    ...
    def apply_face_detection(self):
        if self.picture:
            self.apply_face_detection_on_one_image(self.picture, ViewDetection.INDEX_ROW_IMAGES, 0)
            self.info = self.model.get_info_about_computing()
            tk.Label(self.root, text=self.info).grid(row=ViewDetection.INDEX_ROW_IMAGES + 1, column=0, padx=2, pady=2)
        if self.directory_path:
            row = ViewDetection.INDEX_ROW_IMAGES
            column = 0
            nb_images = 0
            start = time.time()
            for filename in os.listdir(self.directory_path):
                ...
                if os.path.isfile(f):
                    ...
                    column += 1
                    nb_images+=1
                    if column == ViewDetection.NB_IMAGES_PER_COLUMN:
                        row += 2
                        column = 0
            end = time.time()
